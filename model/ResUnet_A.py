...
class Resblock(nn.Module):
    def __init__(self, in_channels, out_channels, block_size=7, p_value=0.5):
        ...
        self.shortcut = nn.Sequential(
                    LayerNorm(in_channels, eps=1e-6, data_format="channels_first"),
                    nn.Conv3d(in_channels, out_channels, kernel_size=1, padding=0, stride=1)
                    )
        self.ECA = EfficientChannelAttention(in_channels=out_channels)
...
class ResUnetA(nn.Module):
    def __init__(self, img_size=128, channels=1, classes=1, filtersize=[64, 128, 256, 512], up_mode="trilinear", check_sigmoid=False):
        ...
        for idx in range(self.depth - 1):
            self.down_layer = nn.Sequential(
                ...
                nn.Conv3d(self.filtersize[idx], self.filtersize[idx+1], kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=0)
            )
        ...
        for idx in range(self.depth-1, 0, -1):
            self.Up = nn.Sequential(
                LayerNorm(self.filtersize[idx], eps=1e-6,  data_format="channels_first"),
                nn.ConvTranspose3d(self.filtersize[idx], self.filtersize[idx-1], kernel_size=(1, 2, 2), stride=(1, 2, 2), padding=0)
