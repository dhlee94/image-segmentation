...
def main():
    ...
    image_lists = [os.path.join(args.image_path, 'img', images.split('.')[0]+'.jpg') for images in os.listdir(os.path.join(args.image_path, 'mask'))]
    label_lists = [os.path.join(args.image_path, 'mask', images) for images in os.listdir(os.path.join(args.image_path, 'mask'))]
    image_lists = np.array(image_lists)[:2000]
    label_lists = np.array(label_lists)[:2000]
