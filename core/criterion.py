...
class DiceLoss(nn.Module):
    ...
    def _one_hot_encoder(self, input_tensor):
        ...
        target_list = torch.stack(target_list, dim=1)
        return target_list

    def _dice_loss(self, score, target, weight, average):
        score = flatten(score)
        ...
        loss = (2 * intersect + self.bias) / (z_sum + y_sum + self.bias)
        loss = 1 - loss
        for idx in range(self.n_classes):
            loss[idx] *= self.weight[idx]
        if average:
            return loss.mean() 
        else:
            return loss.sum()
    ...
    def forward(self, inputs, target):
        ...
        loss = self._dice_loss(inputs, target, self.weight, self.average)
        return loss
...
class DIoUloss(nn.Module):
    ...
    def forward(self, score, target):
        ...
        for idx in range(D):
            ...
            union = torch.clamp((box1_area + box2_area - inter), min=0)
        
            center_x1 = (score[:, idx, 2] + score[:, idx, 0]) / 2
            center_y1 = (score[:, idx, 3] + score[:, idx, 1]) / 2
            center_x2 = (target[:, idx, 2] + target[:, idx, 0]) / 2
            center_y2 = (target[:, idx, 3] + target[:, idx, 1]) / 2

            out_min_x = torch.min(score[:, idx, 0],target[:, idx, 0])
            out_min_y = torch.min(score[:, idx, 1],target[:, idx, 1])
            out_max_x = torch.max(score[:, idx, 2],target[:, idx, 2])
            out_max_y = torch.max(score[:, idx, 3],target[:, idx, 3])        

            inter_diag = torch.sqrt((center_x2 - center_x1)**2 + (center_y2 - center_y1)**2)
            outer_diag = torch.sqrt((out_max_x - out_min_x)**2 + (out_max_y - out_min_y)**2)
            inter_diag = torch.clamp(inter_diag, min=1e-6)
            outer_diag = torch.clamp(outer_diag, min=1e-6)
            dious = 1- inter / (union+self.eps) + (inter_diag) / outer_diag
            Diou_acc += torch.clamp(dious,min=-1.0,max = 1.0)
        return (Diou_acc / D).mean()
    
class MulticlassIoUCELoss(nn.Module):
    def __init__(self, alpha=0.25, delta=0.01):
        super(MulticlassIoUCELoss, self).__init__()
        self.alpha = alpha
        self.delta = delta
        self.IoULoss = DIoUloss()
        self.CrossEntropy = nn.MSELoss()
    def forward(self, score, target):
        IoULoss = self.IoULoss(score, target)
        cce_loss = self.CrossEntropy(score, target)        
        loss = self.delta * self.alpha * cce_loss + (1 - self.alpha) * IoULoss
        return loss
    
class MultiClassDiceCELoss(nn.Module):
    def __init__(self, num_classes=1, alpha=0.25, ignore_label=255, bias=1e-5, weight=None):
        super(MultiClassDiceCELoss, self).__init__()
        self.weight = weight
        self.alpha = alpha
        self.num_classes = num_classes
        self.CrossEntropy = DistanceLoss(num_classes, loss_function=nn.CrossEntropyLoss, softmax=False, weight=weight)
        #self.CrossEntropy = FocalLoss(gamma=2, alpha=None)
        self.DiceLoss = DiceLoss(num_classes, bias=bias, softmax=True)
    def forward(self, score, target):
        score = score
        target = target
        dice_loss = self.DiceLoss(score, target)
        cce_loss = self.CrossEntropy(score, target)
        loss = self.alpha * cce_loss + (1 - self.alpha) * dice_loss
        return loss
